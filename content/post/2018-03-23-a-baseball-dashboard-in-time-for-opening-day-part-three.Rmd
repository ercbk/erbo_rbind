...
In [part one](https://erbo.rbind.io/blog/2018-03-09-a-baseball-dashboard-in-time-for-opening-day-part-one/), we collected the data and performed a little EDA. In [part two](https://erbo.rbind.io/blog/2018-03-21-a-baseball-dashboard-in-time-for-opening-day-part-two/), we did some calculations and prepared the data to be visualized. Now, we get to ***see*** the fruits of our labor. You'll need these objects from part two: `display_table`, `jaws_group`, `war_group`, and `war_combo_avg`.
...
The line chart shows the player's WAR for every season he was a Red. Hovering over each point displays its value. The horizontal dashed line is one of two vales: the median WAR per season of pitchers or the median WAR per season of position players. There were no real issues of note in this implementation.
...
Placing this HTML at the beginning of `dashboardBody` will allow your to choose the colors of various elements of the header.
...
Normally, while the dashboard is waiting for user input, an error will displayed in place of the charts. Placing this code at the beginning of `dashboardBody` will hide this error. The code should be on the same level as `tags$head(` above.
...
This dashboard is actually only one item in a more tricked-out version that I finished a few weeks ago. If you'd like to take this even further, you can see my other dashboard [here](https://erbo.shinyapps.io/jaws4/) and the code is on my github. The processing scripts are a little rough, so be prepared. It was a learning experience `r emo::ji("blush")`. Hopefully, I'll get around to making them more presentable this fall.

This project combined two loves mine: baseball and  r -- which I really enjoyed. There wasn't any complex modeling but it really challenged me in ways I didn't think it would. The visual design decisions were more difficult than I anticipated. It also allowed me to level-up in `purrr` and `tidyeval`. Hope you enjoyed it. 
