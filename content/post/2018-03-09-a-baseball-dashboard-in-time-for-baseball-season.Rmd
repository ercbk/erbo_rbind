...
We can now turn our attention to making a decision on our central quandary: how long should our tenure requirement be? If we look at tables of the different values, we can calculate the percentage of players that would remain at each cutoff.
...
Looking at the nominees, it turns out Scott Rolen only played four seasons for the Reds. The goal of this project is to evaluate nominees, so four years would be the necessary cutoff in order for Rolen to be included. If Rolen wasn't part of this class, I'd consider five years but wouldn't go above five.

So the inductees that didn't make the cut were the following: Billy Werber, Bill McKechnie, and Wayne Granger. The Wright boys, George and Harry, also aren't in there. They played with the Reds before 1871 and their WAR wasn't available.
...
Next, we need to figure out the primary positions for players as they tend to play multiple positions during their career. When comparing JAWS scores between inductees and nominees, it's usually done by position. So, if were to examine Scott Rolen's case, we'd look at his JAWS score and compare it to other Reds third basemen such as Frank Robinson or Chris Sabo. 
...
posDat_i <- map2_dfr(indWar$playerId, indWar$yearId, function(x,y) filter(Fielding, playerID == x & yearID == y)) %>% 
...
# Jim O'Toole's Baseball-Reference ID in the Fielding data set is incorrect but he was a pitcher his whole career. Double-checked. Adding him to the df.
# Renaming playerID to join with another dataframe later
setdiff(indWar$playerId, posDat_i$playerID)
posDat_i <- posDat_i %>% 
...
posDat_n <- map2_dfr(nomWar$playerId, nomWar$yearId, function(x,y) filter(Fielding, playerID == x & yearID == y)) %>% 
      filter(teamID == "CIN" | teamID == "CN1" | teamID == "CN2")
