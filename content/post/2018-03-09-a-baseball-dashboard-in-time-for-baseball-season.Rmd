...
Administrative personnel were included in our members dataframe. They have no WAR values so they've been removed in the filtering process. Some players aren't in `indID` because they had accent marks in their names in `members`. There's also a father/son duo who have the same names in `idTT` but have Sr/Jr suffixes in `members`. All of these players will be added to `indID`.
...
Not many people have original names, including baseball players, so we need to remove the extra Pete Rose (Jr.), Joe Morgan, Mike McCormick, and George Wright. Sparky Anderson and Fred Hutchinson are managers so they can be dropped as well.
